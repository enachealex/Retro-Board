...
const App = () => {
  ...
  const saveRoleLabels = async () => {
    try {
      await axios.put(`${API_URL}/role-labels`, { labels: editingLabels }, authHeaders(token));
  ...
  const addRoleLabel = async () => {
    ...
    if (!key || !val) return;
    try {
      await axios.post(`${API_URL}/role-labels`, { role_key: key, label: val }, authHeaders(token));
  ...
  const deleteRoleLabel = async (key) => {
    try {
      await axios.delete(`${API_URL}/role-labels/${key}`, authHeaders(token));
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/users`, authHeaders(token));
  ...
  const addBoardMember = async (boardId, userId) => {
    try {
      await axios.post(`${API_URL}/boards/${boardId}/members`, { userId }, authHeaders(token));
  ...
  useEffect(() => {
    ...
    socket.on("board:update", (data) => {
      if (data.boardId && activeBoard && data.boardId === activeBoard.id) {
    ...
    socket.on("boards:update", (data) => {
      if (data.boards) {
        if (isSuperUser) {
          const allowed = (data.boards || []).filter(isBoardAllowed);
          setBoards(allowed);
          if (activeBoard) {
          ...
          if (activeBoard) {
            const updated = (data.boards || []).find(b => b.id === activeBoard.id);
  ...
  const fetchBoards = async () => {
    ...
    try {
      ...
      const allowed = (res.data || []).filter(isBoardAllowed);
      setBoards(allowed);
      if (allowed.length > 0) {
                              ...
                              >
                                ...
                                <div className="board-users-info">
                                  ...
                                  <span className="board-users-role">
                                    {u.is_overlord ? 'Overlord' : u.is_master ? 'Iron Fist' : u.is_admin ? 'Admin' : 'Member'}
